...
- `TTS_MOCK_MODE=false`, `TTS_ENABLED=true` — voice requests use Respeecher when credentials are configured
...
Real speech is enabled by default for voice requests; configure the selected provider. With no Respeecher key, the app returns text without fabricated audio. For Respeecher:
