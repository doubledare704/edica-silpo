...
async def create_cart_node(state: SilpoAgentState) -> dict[str, Any]:
    ...
    if settings.MCP_MOCK_MODE:
        ...
        try:
            cart_url = await mcp_product_service.create_cart(state.get("mcp_products", []))
    ...
    logger.info("create_cart done total=%s cart_url=%s summary_chars=%d", total_price, cart_url, len(summary_message))
    return {
        "cart_url": cart_url,
