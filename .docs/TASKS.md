...
- Bumped to `silpo-py-mcp>=0.4.0` (live-shape reconciliation): `fetch_replacements` expands the new `items[].replacements[]` envelope (legacy `replacement` still accepted); profile name falls back to `firstName+lastName`; saved-address text synthesized from `city/street/building/apartment` with `latitude/longitude` coordinates; coupon/personal-promo normalizers fall back to `description`/`rewardValue`/`endDateTime`/`endDate` for empty live fields.
...
- Bumped to `silpo-py-mcp>=0.5.0` (server release-1.110.1): `fetch_similar` passes required `delivery_type`/`timeslot_start`/`timeslot_end` to `get_similar_products` (old optional-delivery call raised `TypeError`); `FakeClient` mirrors the strict signature and asserts context propagation plus a real-mock `fetch_similar` regression test.
- Budget-only weekly plan: `plan_meals` creation was already BUDGET-gated, but seed consumers were not — `plan_domain_logic` and the picker now honor `meal_plan.shopping_seed` only for `BUDGET` (stale seeds never leak into party/office/gourmet), non-weekly `plan_meals` explicitly clears `meal_plan` + `calculated_items`, `BudgetDomainPlanner.format_summary` uses weekly wording only for `BUDGET`, SSE `node_complete` serializes `meal_plan` only for `BUDGET`, and the frontend forwards `intent` (`AgentTimeline` → `+page` → `CartCard`) with the 7-day section rendering only for `intent === 'budget'`.
...
- [x] Per-query cart replace: `create_cart` always clears a reused server cart before upsert (replace semantics per query, clear failures warn-and-continue without killing the write); the old `detail.items` emptiness check missed live carts that nest products under `shipments[].products`, which mixed old+new items and corrupted the verified total.
